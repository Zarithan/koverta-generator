...
st.title("📝 Генератор коверата (обе стране)")

ime = st.text_input("Прималац / Назив")
...
if st.button("Генериши обе стране"):
    ...
    if rezultat.empty:
        st.error("Место није пронађено у бази.")
    else:
        ...
        popuni_prvu_stranu("ПРВА СТРАНА КОВЕРТЕ.docx", prva_out, ime, adresa, mesto, post_broj_fmt)
        popuni_drugu_stranu("ДРУГА СТРАНА КОВЕРТЕ.docx", druga_out, broj_predmeta, ime, adresa)
